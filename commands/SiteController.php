...
protected function buildDailyWeekComparison($table, $sumField, $weeksAgoStart)
{
    ...
    for ($i = 0; $i < 7; $i++) {
        ...
        $seriesA[] = [
            'category' => $dayNames[$i],
            'sum' => isset($dataA[$dA]) ? round($dataA[$dA], 2) : ($dA <= $endA ? 0 : null),
            'date' => Yii::$app->formatter->asDate($dA, 'd MMMM'),
        ];
        $seriesB[] = [
            'category' => $dayNames[$i],
            'sum' => round($dataB[$dB] ?? 0, 2),
...
protected function buildDailyMonthComparison($table, $sumField, $monthsAgoStart)
{
    ...
    for ($d = 1; $d <= $maxDays; $d++) {
        ...
        $seriesA[] = [
            'category' => (string) $d,
            'sum' => $dateA === null ? null : ($dateA <= $endA ? round($dataA[$dateA] ?? 0, 2) : null),
            'date' => $dateA === null ? null : Yii::$app->formatter->asDate($dateA, 'd MMMM'),
        ];
        $seriesB[] = [
            'category' => (string) $d,
            'sum' => $dateB === null ? null : round($dataB[$dateB] ?? 0, 2),
...
/**
 * Сумма по каждому часу (00..23) за одну календарную дату.
...
/**
 * Сумма по каждому календарному дню в диапазоне [$dateFrom, $dateTo].
 * Возвращает ['2026-07-20' => 1234.5, ...] — по датам, у которых есть данные.
 */
