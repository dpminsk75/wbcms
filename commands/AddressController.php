...
class AddressController extends Controller
{
    ...
    private function queryDadataSuggest($address, $apiKey)
    {
        ...
        if ($httpCode !== 200) {
            $payload = ['error' => 'HTTP_ERROR', 'code' => $httpCode, 'message' => $result];
            if (in_array($httpCode, [401, 402, 403, 429], true)) {
        ...
        foreach ($records as $record) {
            ...
            if ($detectedRegion) {
                ...
                $db->createCommand()->update('dadata_address_cache', [
                    ...
                    'city' => $detectedCity,
                    'federal_district' => $federalDistricts[$detectedRegion] ?? $record['federal_district'],
