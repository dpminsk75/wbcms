...
class SeoAnalyzerService
{
    ...
    public function buildPrompt(int $nmID, int $companyId, string $dateFrom, string $dateTo): ?array
    {
        ...
        $defaultSystem = "Ты — SEO-специалист Wildberries. На основе title/description карточки, характеристик и кластеров поисковых фраз предложи улучшения по правилам WB.\n"
            ...
            . "Характеристики: заполни максимум полей. Если кластер содержит \"для беременных\" и т.п. — установи значение в характеристиках (дает индексацию без спама в тексте).\n"
            . "Описание {$descMin}-{$descMax} симв: вприлетай ключи из оставшихся кластеров в естественные предлоги. WB понимает леммы и синонимы — точные вхождения не нужны. Пиши грамотно: \"летнее женское платье\" вместо \"платье женский летний\" — лемма зачтется.\n"
            . "Принцип «Одно слово — один раз\": если слово уже в Заголовке/Категории (subjectName) — не повторяй 10 раз в описании. Повтор не ростет позиции, а риск пессимизации за спам. Используй синонимы.\n"
            . "Морфология: не коверкай фразы. Требования: заголовок до 60 симв, описание {$descMin}-{$descMax}. Не выдумывай характеристики, не добавляй ключей вне списка кластеров и целевых.\n"
