...
class AggSalesReportController extends Controller
{
    public function actionCompare()
    {
        ...
        $rawData = Yii::$app->db->createCommand("
            SELECT 
                ...
                SUM(ppvz_for_pay_sum) as for_pay_amount
            FROM agg_sales_daily_sku
            WHERE YEAR(sale_date) IN (2024, 2025, 2026) AND nmID != 0
            GROUP BY y, m
        ...
        $pmSummaryRaw = Yii::$app->db->createCommand("
            SELECT 
                SUM(CASE WHEN YEAR(sale_date) = :y0 THEN sales_qty ELSE 0 END) as qty_0,
                SUM(CASE WHEN YEAR(sale_date) = :y0 THEN retail_amount_sum ELSE 0 END) as amt_0,
                SUM(CASE WHEN YEAR(sale_date) = :y1 THEN sales_qty ELSE 0 END) as qty_1,
                SUM(CASE WHEN YEAR(sale_date) = :y1 THEN retail_amount_sum ELSE 0 END) as amt_1,
                SUM(CASE WHEN YEAR(sale_date) = :y2 THEN sales_qty ELSE 0 END) as qty_2,
                SUM(CASE WHEN YEAR(sale_date) = :y2 THEN retail_amount_sum ELSE 0 END) as amt_2
            FROM agg_sales_daily_sku
            WHERE (YEAR(sale_date) = :y0 AND MONTH(sale_date) = :m)
               OR (YEAR(sale_date) = :y1 AND MONTH(sale_date) = :m)
               OR (YEAR(sale_date) = :y2 AND MONTH(sale_date) = :m)
        ", [
        ...
        $cmSummaryRaw = Yii::$app->db->createCommand("
            SELECT 
                SUM(CASE WHEN YEAR(sale_date) = :y0 AND DAY(sale_date) <= :d0 THEN sales_qty ELSE 0 END) as qty_0,
                SUM(CASE WHEN YEAR(sale_date) = :y0 AND DAY(sale_date) <= :d0 THEN retail_amount_sum ELSE 0 END) as amt_0,
                SUM(CASE WHEN YEAR(sale_date) = :y1 AND DAY(sale_date) <= :d1 THEN sales_qty ELSE 0 END) as qty_1,
                SUM(CASE WHEN YEAR(sale_date) = :y1 AND DAY(sale_date) <= :d1 THEN retail_amount_sum ELSE 0 END) as amt_1,
                SUM(CASE WHEN YEAR(sale_date) = :y2 AND DAY(sale_date) <= :d2 THEN sales_qty ELSE 0 END) as qty_2,
                SUM(CASE WHEN YEAR(sale_date) = :y2 AND DAY(sale_date) <= :d2 THEN retail_amount_sum ELSE 0 END) as amt_2
            FROM agg_sales_daily_sku
            WHERE MONTH(sale_date) = :m AND YEAR(sale_date) IN (:y0, :y1, :y2)
        ", [
        ...
        for ($i = 1; $i <= 12; $i++) {
            $processedData[$i] = [
                ...
                'qty_2024' => 0, 'amount_2024' => 0, 'for_pay_2024' => 0,
                'qty_2025' => 0, 'amount_2025' => 0, 'for_pay_2025' => 0,
                'qty_2026' => 0, 'amount_2026' => 0, 'for_pay_2026' => 0,
            ];
        }
        ...
        foreach ($rawData as $row) {
            ...
            $processedData[$m]["for_pay_$y"] = (float)$row['for_pay_amount'];
        }
        ...
        $topIdsRows = Yii::$app->db->createCommand("
            SELECT nmID FROM (
                (SELECT nmID FROM agg_sales_daily_sku WHERE MONTH(sale_date) = :lm AND YEAR(sale_date) = 2024 AND nmID != 0 GROUP BY nmID ORDER BY SUM(retail_amount_sum) DESC LIMIT 20)
                UNION
                (SELECT nmID FROM agg_sales_daily_sku WHERE MONTH(sale_date) = :lm AND YEAR(sale_date) = 2025 AND nmID != 0 GROUP BY nmID ORDER BY SUM(retail_amount_sum) DESC LIMIT 20)
                UNION
                (SELECT nmID FROM agg_sales_daily_sku WHERE MONTH(sale_date) = :lm AND YEAR(sale_date) = 2026 AND nmID != 0 GROUP BY nmID ORDER BY SUM(retail_amount_sum) DESC LIMIT 20)
                UNION
                (SELECT nmID FROM agg_sales_daily_sku WHERE MONTH(sale_date) = :cm AND YEAR(sale_date) = 2024 AND nmID != 0 GROUP BY nmID ORDER BY SUM(retail_amount_sum) DESC LIMIT 20)
                UNION
                (SELECT nmID FROM agg_sales_daily_sku WHERE MONTH(sale_date) = :cm AND YEAR(sale_date) = 2025 AND nmID != 0 GROUP BY nmID ORDER BY SUM(retail_amount_sum) DESC LIMIT 20)
                UNION
                (SELECT nmID FROM agg_sales_daily_sku WHERE MONTH(sale_date) = :cm AND YEAR(sale_date) = 2026 AND nmID != 0 GROUP BY nmID ORDER BY SUM(retail_amount_sum) DESC LIMIT 20)
            ) as t
        ...
        if (!empty($topIdsRows)) {
            ...
            $detailsRaw = Yii::$app->db->createCommand("
                SELECT 
                    ...
                    AVG(s.retail_amount_sum / s.sales_qty) as avg_price
                FROM agg_sales_daily_sku s
                LEFT JOIN wbcards c ON c.nmID = s.nmID
                WHERE s.nmID IN ($idsString) AND MONTH(s.sale_date) IN (:lm, :cm) AND YEAR(s.sale_date) IN (2024, 2025, 2026)
