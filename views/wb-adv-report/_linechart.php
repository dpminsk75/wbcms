...
am5.ready(function() {
    ...
    var xAxisTL = Linechart.xAxes.push(am5xy.DateAxis.new(rootTime, {
        maxDeviation: 0.2,
    ...
    xAxisTL.get("renderer").labels.template.setAll({
...
// yAxisMoney
    ...
    yAxisQty.get("renderer").labels.template.setAll({ fill: am5.color(0x5067de) });
    yAxisMoney.get("renderer").labels.template.setAll({ fill: am5.color(0xf96666) });


window.setGraphIntervalTL = function(unit, element) {
    if (typeof xAxisTL !== 'undefined') {
        xAxisTL.set("groupInterval", { timeUnit: unit, count: 1 });
    }

    const container = element 
        ? element.closest('.btn-group') 
        : document.getElementById('timeline_buttons');
...
//    series.data.setAll(data);
    ...
    window.setGraphIntervalTL(initialUnit);

    // Продажи на ЛЕВУЮ ось
    createSeries("Клики", "clicks", yAxisQty, am5.color(0xf965cf));
    createSeries("Корзины", "atbs", yAxisQty, am5.color(0x660ec8));
    createSeries("Заказы", "orders", yAxisQty, am5.color(0x5067de), false, 1, true);

    createSeries("Расходы",   "sum", yAxisMoney, am5.color(0xf96666), true, 3, true);
